...
%
% \bdiag[s] start diagram. If s is given, the H diagram is in the middle (otherwise it is shifted halfway to the vacuum)
%
...
% \dcurt{<from node>}{<through node>}{<to node>} -> curved "--->" over three nodes! (don't use for linear - will fail 
%                                                                             (bloody ubuntu10.04 and old TexLive09...)
% \dcurtr{<from node>}{<through node>}{<to node>} ->   curved "--->" over three nodes (reverse bend)
%
...
\newcommand{\dcurt}[3]{
  \draw[ph_line] let \p1 = (#1), \p2 = (#2)
    in \pgfextra{
          \pgfmathsetmacro{\ang}{ 90-2*atan((\y2-\y1)/(\x2-\x1))}
       }
    (#1) to [out=-\ang,in=-90] (#2);
  \draw[ph_line] let \p1 = (#2), \p2 = (#3)
    in \pgfextra{
        \pgfmathsetmacro{\ang}{90+2*atan((\y2-\y1)/(\x2-\x1))}
       }
    (#2) to [out=90,in=\ang] (#3);
}
\newcommand{\dcurtr}[3]{
  \tikzset{ph_line/.style={ph_liner}}
  \dcurt{#3}{#2}{#1}
