...
\FPset\xcoorH{\xcoord}
\FPeval\yvac{clip(\ycoor+0.5*\sc)} \FPeval\xvac{clip(0.25*\sc)} \FPeval\xdvac{clip(0.5*\sc)}
\newcounter{stoer} 
...
\FPeval\xx{clip(\xcoorend+\xvac/2)} 
\node[inner sep=0pt,minimum size=0pt] (#2v2) at (\xx,\yvac) {};
\FPeval\xx{clip(\xcoorend-\xdvac/2)}
\node[inner sep=0pt,minimum size=0pt] (#2vd1) at (\xx,\ycoord) {};
\FPeval\xx{clip(\xcoorend+\xdvac/2)}
\node[inner sep=0pt,minimum size=0pt] (#2vd2) at (\xx,\ycoord) {};
}
...
\newcommand{\dmovex}[1]{\FPeval\xcoorH{clip(\xcoorH+#1*0.25*\sc)}}
\newcommand{\dmoveT}[1]{\FPeval\xcoor{clip(\xcoor+#1*0.25*\sc)}}
\newcommand{\dmoveTd}[1]{\FPeval\xcoordg{clip(\xcoordg+#1*0.25*\sc)}}
